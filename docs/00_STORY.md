...
**7. 과거 장기 스폰서 캠페인이 데이터 근거 없이 진행되어 기대만큼 성과를 내지 못한 경험이 있다.**
Cloud Alpacas는 이전에도 장기 스폰서십 캠페인을 진행한 적이 있지만, 실제 팬층과의 궁합을 데이터로 검증하지 않고 시작해 기대만큼의 성과로 이어지지 못했다. 이 경험이 이번에 Fan 360 데이터를 근거로 후보를 찾고, 단기 Collaboration으로 먼저 검증하는 방식을 택하게 된 배경이다.
...
과거에 진행했던 장기 스폰서십 캠페인이 데이터 근거 없이 추진되어 기대만큼 성과를 내지 못했던 경험(§2 Pain Point 7)도, 이번에는 Fan 360 데이터를 근거로 후보를 찾고 단기 Collaboration으로 먼저 검증한 뒤 장기 계약으로 발전시키는 방식을 택하게 된 배경이다.
...
이 매니저는 Fan 360 데이터를 살펴보다가, 최근 여성 팬의 유입이 크게 늘었다는 것을 발견한다. 그런데 자세히 들여다보니 팬 수 증가와 달리 이 팬층의 구매력과 재방문율은 오히려 낮다 — 단순히 팬 수를 늘리는 것만으로는 구단의 가치가 커지지 않는다는 신호다.

**2. 팬층 특성 → 새로운 경험/상품 필요성 → 궁합 좋은 기업 가설**

이 매니저는 여성 팬의 구매와 재방문을 높일 수 있는 새로운 경험/상품이 필요하다고 판단한다. Fan 360을 다시 분석해보니, 이 팬층은 캐릭터·라이프스타일 계열 콘텐츠에 유독 높은 관심을 보인다. 이 특성을 근거로 "이런 팬층과 잘 맞는 기업/브랜드는 어떤 곳일까?"라는 가설을 세우고, 이 팬층과 접점이 있는 브랜드를 찾기 시작한다 — 그 과정에서 산리오(Sanrio)가 후보로 등장한다.

> 정확한 증가율·구매력 격차 등 실제 수치는 아직 확정되지 않았다(TBD) — `docs/data/P2_DUMMY_DATA_MASTER.md`의 Fan Insight/Dummy Data 설계 단계에서 채운다. "구단이 먼저 제안할 후보"가 감이 아니라 데이터 기반 가설이라는 점이 이 단계의 핵심이다.

**3. 가설 → 후보 발굴 → Business Fit 검토**

가설에 맞는 실제 기업 후보(예: 산리오)를 찾고, 그 기업의 고객층이 정말 Cloud Alpacas 팬층과 겹치는지 검토한다. "유명한 회사"가 기준이 아니라 "우리 팬과 Fit이 높은 회사"가 기준이다.
...
- **Collaboration 실패 후 대응**: §8.3의 6~7단계(Collaboration 성과 → 다음 의사결정)에서 성과가 기대에 못 미칠 경우 이 매니저가 성과를 분석하고 재검토하거나 관계 종료를 판단할 수 있다는 것은 Story 수준에서는 유효하다. 다만 이 대응을 실제로 어떻게 실행하는지는 이번 8/21 구현 범위에 포함하지 않는다(`04_DEMO.md` §7 참고).
- **Collaboration 성공 기준/KPI**: "성과가 좋다/기대에 못 미친다"를 판단할 구체적인 기준이나 지표는 현재 확정하지 않았다(TBD) — Future Scope로 남긴다.
...
| Collaboration 성과가 좋음 | 장기 Partnership/Sponsorship 제안 *(Future Scope — 8/21 구현 범위 밖, §8.4)* |
| Collaboration 성과가 기대에 못 미침 | 재검토 또는 관계 종료 *(판단 기준 TBD, Future Scope — 8/21 구현 범위 밖, §8.4)* |
